...
static struct free_block *free_lists[MAX_RANK + 1];
static int            free_cnt[MAX_RANK + 1];
static int            max_free_rank = 0;

/* ------------------------------------------------------------------ */
/*  Helpers                                                           */
/* ------------------------------------------------------------------ */

static int page_index(void *p)
{
    return (int)(((char *)p - (char *)mem_base) / PAGE_SIZE);
}

static void *page_addr(int idx)
{
